...
[1] Tạo ExamSession
        ↓
[2] Tạo Assignment (per mã đề) + Upload DB SQL + Given API (URL hoặc zip)
        ↓
...
- `PUT /assignments/{id}/resources` — upload DB SQL, Given API URL hoặc Given API source zip
...
### [2b] Upload Resources (DB SQL + Given API)
...
| Field | Type | Bắt buộc | Ghi chú |
|---|---|---|---|
| databaseSql | File (.sql) | — | Ít nhất 1 trong 3 phải có |
| givenApiBaseUrl | string | — | HTTP/HTTPS URL hợp lệ — dùng khi given API chạy ngoài |
| givenZip | File (.zip) | — | Source code của given API — Worker tự khởi động khi chấm Q2 |

> **Ưu tiên:** nếu có cả `givenZip` lẫn `givenApiBaseUrl`, `givenZip` được dùng (URL được gán động lúc runtime).
...
```json
{
  ...
  "title": "Mã đề 101",
  "description": "...",
  "databaseSqlPath": "/storage/assignments/.../database.sql",
  "givenApiBaseUrl": null,
  "hasGivenZip": true,
  "createdAt": "..."
}
...
> `hasGivenZip: true` — given API sẽ được tự động khởi động từ zip lúc chấm. FE dùng flag này để hiển thị trạng thái upload.
...
### Given API — zip vs URL
- `givenZip`: upload source code của given API → Worker tự extract và khởi động trên cổng ngẫu nhiên khi chấm
- `givenApiBaseUrl`: URL của given API đang chạy sẵn bên ngoài
- Ưu tiên: `givenZip` > `givenApiBaseUrl`
- Khi dùng `givenZip`, skip check `appsettings.json` chứa URL cố định (URL được inject vào env lúc runtime)
- FE dùng `hasGivenZip` từ `AssignmentDto` để hiển thị trạng thái (không trả về path thực)
